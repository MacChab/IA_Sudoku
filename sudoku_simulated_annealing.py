...
def search(values):
    ...
    while temperature > 0.000009 and try_no < 100000:


        # print("try : ", try_no)
        # print("nb_erreur ", curr_error)
        if curr_error == 2:
            test_values = search_2(values)
            if test_values:
                return test_values
            ...
            for first in range(len(box) - 1):
                ...
                while second < len(box) - 1:
                    ...
                    if new_error == 0:
                        ...
                        s = values[box[first]]
                        values[box[first]] = values[box[second]]
                        values[box[second]] = s
                        curr_error = new_error

                    elif np.random.binomial(size = 1,n = 1, p = np.exp(-1*(new_error - curr_error)/temperature))[0]:
                        s = values[box[first]]
                        values[box[first]] = values[box[second]]
                        values[box[second]] = s
                        curr_error = new_error
...
def search_2(values):
    """trouve quel rangee ou colonne a les erreurs essais les 4 facon de le regler et
    retourne values si ca marche et false si non"""
    ...
    #trouve les range ou colonne qui on des erreur et leurs valeurs
    for unit in rows:
        d = digits
        for i in unit:
            d = d.replace(values[i], '')
        for i in range(0, len(d)):
            row_erreur.append(index_row)
        index_row += 1
        if len(d) > 0:
            digit_final.append(d)
    index_col = 0
    for unit in cols:
        d = digits
        for i in unit:
        ...
        for i in range(len(d)):
            col_erreur.append(index_col)
        index_col += 1
        if len(d) > 0:
            digit_final.append(d)

    # si il y a une erreur dans une colonne et une dans une ligne cest que cest impossible
    if len(col_erreur) == 1:
        return False

    # trouve les cases avec les valeurs a echanger et essaie chaque des 4 permutations
    else:
        if len(row_erreur):
            row1 = []
            for i in rows[row_erreur[0]]:
                if values[i] in digit_final[1]:
                    row1.append(i)
            row2 = []
            for i in rows[row_erreur[1]]:
                if values[i] in digit_final[0]:
                    row2.append(i)
            for r1 in row1:
                for r2 in row2:
                    test_values = values.copy()
                    temp = test_values[r1]
                    test_values[r1] = test_values[r2]
                    test_values[r2] = temp
                    if solved(test_values):
                        return test_values
    return False
